...
console.log("step: hearing a voice out");
{
  ...
  const first = run(fresh, { kind: "say", voice: "charles", text: previewText("charles", []) });
  assert("say: synthesize under -1 in that voice, then play — on a player answering to -1", first.commands.join() === "synthesize -1 charles,play@-1" && sounding(first.state) === "charles@-1" && first.state.next === -2);
  const request = step(fresh, { kind: "say", voice: "charles", text: previewText("charles", []) }).commands[0];
  assert("the request carries the preview phrase, in the voice's own name", request?.kind === "worker" && request.message.kind === "synthesize" && request.message.text.text === previewText("charles", []).text);
  ...
  const replaced = run(first.state, { kind: "say", voice: "paul", text: previewText("paul", []) });
  assert("say while sounding: the old request withdrawn, a fresh id, the new one played", replaced.commands.join() === "cancel -1,synthesize -2 paul,play@-2" && sounding(replaced.state) === "paul@-2" && replaced.state.next === -3);
  ...
  const next = run(hushed.state, { kind: "say", voice: "jane", text: previewText("jane", []) });
  assert("the next say takes the next id down", next.commands.join() === "synthesize -3 jane,play@-3");

  throws("cancelled for the sounding preview is a previewer bug", () => step(replaced.state, worker({ kind: "cancelled", unitId: -2 })));
  throws("a refused preview is a previewer bug", () => step(replaced.state, worker({ kind: "refused", request: { kind: "synthesize", unitId: -2, text: previewText("paul", []), voice: "paul" }, phase: "idle" })));
  throws("a refused synthesize of a replaced preview is one too", () => step(replaced.state, worker({ kind: "refused", request: { kind: "synthesize", unitId: -1, text: previewText("charles", []), voice: "charles" }, phase: "idle" })));
  ...
  const others = run(replaced.state, worker({ kind: "refused", request: { kind: "load" }, phase: "ready" }), worker({ kind: "progress", progress: { loadedBytes: 1, totalBytes: 2 } }), worker({ kind: "refused", request: { kind: "synthesize", unitId: 0, text: previewText("charles", []), voice: "charles" }, phase: "idle" }));
...
console.log("driver: a stub port, one real player per preview, a device of its own");
{
  ...
  previewer.say("charles", previewText("charles", []));
  const device = devices()[0];
  if (device === undefined) throw new Error("the first say did not open a device");
  assert("the first say opens the device and resumes it — the tap's gesture — and asks for the phrase", devices().length === 1 && device.calls.join() === "resume" && said() === "synthesize -1 charles" && changes.join() === "charles");
  ...
  assert("the phrase ends: silent, and the picker told", previewer.state().sounding === null && changes.join() === "charles," && device.calls.at(-1) === "suspend");

  previewer.say("paul", previewText("paul", []));
  previewer.say("jane", previewText("jane", []));
  assert("a second voice tapped over the first: the first withdrawn, the second asked", said() === "synthesize -1 charles,synthesize -2 paul,cancel -2,synthesize -3 jane" && changes.join() === "charles,,paul,jane");
  ...
  assert("the sounding preview's frame plays", device.sources.length === 2 && previewer.state().sounding?.voice === "jane");
  previewer.hush();
  ...
  previewer.say("vera", previewText("vera", []));
  const before = changes.length;
  previewer.dispose();
  assert("dispose: the request withdrawn, the port unheard, the device closed, nobody told", said().endsWith("synthesize -5 vera,cancel -5") && listeners.size === 0 && device.calls.at(-1) === "close" && changes.length === before);
}
