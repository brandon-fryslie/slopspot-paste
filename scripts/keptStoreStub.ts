// The device's kept-audio store as three maps, keeping exactly the entries, records and audio
// it is given: the in-memory KeptStore every kept-audio check drives in place of IndexedDB.
...
export const memoryStore = ({ room = Number.POSITIVE_INFINITY }: { room?: number } = {}) => {
  ...
  const store: KeptStore = {
    ...
    put: async (entry, record, bytes) => {
      if (held() - (ledger.get(entry.key)?.bytes ?? 0) + entry.bytes > room) throw new DOMException("the device is full", "QuotaExceededError");
  ...
  return { store, ledger, records, audio };
};
