...
console.log("step: from idle to the first audio");
{
  ...
  assert(
    ...
    message?.kind === "worker" && message.message.kind === "synthesize" && message.message.unit.text === "Unit 0 says hello." && message.message.voice === "marius",
  );
...
console.log("step: failure is skipped, not waited on");
{
  ...
  throws("a refused synthesize is a scheduler bug", () => step(VOICES, going.state, worker({ kind: "refused", request: { kind: "synthesize", unitId: 2, unit: unitOf(2, ""), voice: "alba" }, phase: "idle" }), speaking(0)));
