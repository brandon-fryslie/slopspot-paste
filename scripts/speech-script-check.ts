...
console.log("\nSpoken notation (slopspot-read-along-a35.5jv):");
{
  ...
  const CASES: Readonly<Record<NotationName, Case>> = {
    ...
    squared: { says: ["x^2 and y²", "X squared and y squared."], leaves: ["the ^2 key"] },
    cubed: { says: ["a^3 + b³", "A cubed plus b cubed."], leaves: ["the ^3 key"] },
    toThePowerOf: { says: ["10^6 and 2^n and x^23", "10 to the power of 6 and 2 to the power of n and x to the power of 23."], leaves: ["a^ caret", "^_^"] },
...
console.log("\nScript hash:");
{
  ...
  const rehashed = <A extends ModelAsset>(asset: A): A => ({ ...asset, sha256: "f".repeat(64) });
  const [same, again, edited, recast, pipelineChanged, tokenizerChanged, weightsChanged, generationChanged, budgetChanged] = await Promise.all([
    scriptHash(said),
    ...
    scriptHash(said, renditionVersions({ ...MODEL_ASSETS, weights: { ...MODEL_ASSETS.weights, maxUnitTokens: MODEL_ASSETS.weights.maxUnitTokens + 1 } })),
  ]);
  ...
  assert("a new generation is another script: its encoder counts the budget", same !== generationChanged);
  assert("a new unit budget is another script", same !== budgetChanged);
  assert("new weights under the same budget cut the same script", same === weightsChanged);
}
