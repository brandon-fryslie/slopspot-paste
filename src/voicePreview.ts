...
export interface PreviewerConfig {
  ...
  // What opens the preview's own audio device: an AudioContext played through a media element
...
export const createPreviewer = (config: PreviewerConfig): Previewer => {
  ...
  const playerOf = (unitId: number): UnitPlayer => {
    ...
    const built = createUnitPlayer({ device: device(), layout: [{ kind: "speech", span: 0 }], onState: (reported) => dispatch({ kind: "player", unitId, state: reported }) });
