...
// so the two conversions below are one array read in each direction over `units`, the
// speech legs by unit. A unit's span of the clock begins where the previous unit's leg ends
// and holds its lead and its leg, so a time inside a gap is the unit the gap precedes at a
// negative offset — exactly how the player plays the gap — and neither direction has a case
// for silence [LAW:dataflow-not-control-flow]. Two caps keep the clock honest: the player's
// offset reads no further than the unit's leg, since while a unit streams its leg is still
// the estimate the clock is laid on; and a seek lands no further into a leg than has been
// heard — its length when measured, its start when a guess — the rule `timeAt` applies to
// a mark, stated once there and once here [LAW:single-enforcer]. The same rule that lays
// the gaps on the clock (`leadsOf`) hands the player the lead-in before each unit, so the
// clock and the audio cannot disagree. The timeline and its unit legs are delivered with
// every view, cut once per manifest, so a reader of the view — the panel's cursor, its
// scrubber, its status line — never builds a second one and the per-frame read is one
// array index.
//
...
import { leadsOf, speechLegs, timelineOfScript, type SpeechLeg, type Timeline } from "./timeline";
...
// Where a unit's span of the clock begins: at the end of the leg before it, and at the top
// for the first.
const sinceOf = (units: ReadonlyArray<SpeechLeg>, unitIndex: number): number => {
  const before = units[unitIndex - 1];
  return before === undefined ? 0 : before.startMs + before.ms;
};

// Where the pipeline's position falls on the conversation's clock: the unit's leg start
// plus the offset, read no further than the leg. A unit the timeline has no leg for is a
// player built over another script and throws [LAW:no-silent-failure].
export const timeOf = (units: ReadonlyArray<SpeechLeg>, at: Position): number => {
  ...
  return leg.startMs + Math.min(at.offsetMs, leg.ms);
};

// The pipeline position a time seeks to: the last unit whose span has begun, and the time
// past that unit's leg start — negative inside its lead, and no further into the leg than
...
export const positionAt = (units: ReadonlyArray<SpeechLeg>, ms: number): Position => {
  const unitIndex = Math.max(0, units.findLastIndex((_, index) => sinceOf(units, index) <= ms));
  ...
  return { unitIndex, offsetMs: Math.min(Math.max(ms, sinceOf(units, unitIndex)) - leg.startMs, heard) };
};
...
export const stateOf = (view: NeuralView): PerformerState => {
  ...
  return { kind: player.kind, atMs: timeOf(view.units, player.at) };
};
