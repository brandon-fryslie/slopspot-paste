...
//
// THE GAP IS THE TIMELINE'S OWN [LAW:single-enforcer]. `gapsOf` says how much silence lies
// before each span of a conversation given the anchors of its spans: GAP_MS where the
// anchor changes — a turn is a run of one anchor, so only a change of speaker gets a gap —
// and none before the first, because there is no turn before it. The timeline lays its
// silence legs from that list. The gap belongs to no unit: it is a leg of the clock in its
// own right, between the speech legs, and nothing that plays or paints a unit owns it.
//
...
// none within a turn.
export const gapsOf = (anchors: ReadonlyArray<string>): ReadonlyArray<number> =>
...
const build = (spans: ReadonlyArray<Span>): Timeline => {
  ...
  const gaps = gapsOf(spans.map((span) => span.anchor));
  ...
  const legs = spans.flatMap((span, index): Leg[] => {
    ...
    return [...(gap > 0 ? [lay(gap, SILENCE)] : []), lay(span.measuredMs ?? chars(span) * rate, speech)];
  });
...
const isSpeech = (leg: Leg): leg is SpeechLeg => leg.content.kind === "speech";
export const speechLegs = (timeline: Timeline): ReadonlyArray<SpeechLeg> => timeline.legs.filter(isSpeech);
...
export const markAt = (timeline: Timeline, ms: number): Mark | null => {
  ...
  return leg === undefined ? null : markIn(leg, ms);
};

// The name of a point on the clock within a speech leg's span: the character its share of
// the leg reaches, the first before the leg begins, the last past its end. The leg is the
// caller's fact — the neural voice knows its unit outright, and a time put to the whole
// timeline would name a neighbour while a unit still streams past its guessed length.
export const markIn = (leg: SpeechLeg, ms: number): Mark => {
...
export const spotAt = (timeline: Timeline, ms: number): Spot | null => {
  ...
  return leg === undefined || !isSpeech(leg) ? null : spotIn(leg, ms);
};

// What the read-along paints at a point on the clock within a speech leg: the leg's
// segment, with the word under the point when the leg is measured.
export const spotIn = (leg: SpeechLeg, ms: number): Spot => {
