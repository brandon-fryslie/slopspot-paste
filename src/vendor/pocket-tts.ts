...
// preserved exactly — moving one is a leak or a use-after-free on the GPU.
...
// The element at `i`, which the model's structure guarantees exists (parallel arrays of
// layers and caches, fixed block lists). A miss is a broken checkpoint, not a case to skip.
const at = <T>(xs: ReadonlyArray<T>, i: number): T => {
  const x = xs[i];
  if (x === undefined) throw new RangeError(`pocket-tts: index ${i} out of ${xs.length}`);
  return x;
};

const dims2 = (shape: ReadonlyArray<number>): [number, number] => {
  const [a, b] = shape;
  if (shape.length !== 2 || a === undefined || b === undefined) {
    throw new RangeError(`pocket-tts: expected rank 2, got shape [${shape.join(", ")}]`);
  }
  return [a, b];
};

const dims3 = (shape: ReadonlyArray<number>): [number, number, number] => {
  const [a, b, c] = shape;
  if (shape.length !== 3 || a === undefined || b === undefined || c === undefined) {
    throw new RangeError(`pocket-tts: expected rank 3, got shape [${shape.join(", ")}]`);
  }
  return [a, b, c];
};
...
export const runSimpleMLPAdaLN = jit(function runSimpleMLPAdaLN(
  ...
  const sEmb = runTimestepEmbedder(at(timeEmbed, 0), s);
  const tEmb = runTimestepEmbedder(at(timeEmbed, 1), t);
  ...
  const [shift, scale] = dims2Arrays(np.split(finalMod, 2, -1));
...
// `np.split` returns a list; these state the count the call asked for.
const dims2Arrays = (parts: np.Array[]): [np.Array, np.Array] => [at(parts, 0), at(parts, 1)];
const dims3Arrays = (parts: np.Array[]): [np.Array, np.Array, np.Array] => [at(parts, 0), at(parts, 1), at(parts, 2)];
...
export function runRope(
  ...
  let [qr, qi] = dims2Arrays(np.split(qReshaped, 2, -1));
  let [kr, ki] = dims2Arrays(np.split(kReshaped, 2, -1));
  qr = np.squeeze(qr, -1);
  qi = np.squeeze(qi, -1);
  kr = np.squeeze(kr, -1);
  ki = np.squeeze(ki, -1);
...
export function runMimiStreamingMultiheadAttention(
  ...
  const [q_, k_, v] = dims3Arrays(np.split(qkv, 3, 1)); // each is [T, H, D]
  ...
  if (isPrefill) {
    ...
    const capacity = at(kvCache.key.shape, 0);
    const cacheMask = np
      .arange(capacity)
      .reshape([-1, 1, 1])
      .less(kvCacheLen.ref);
...
export function runSEANetResnetBlock(
  ...
  for (const layer of block) {
    if (layer === undefined) {
      ...
      // StreamingConv1d
      [v, states[stateIdx]] = runConv1d(layer.conv, states[stateIdx] ?? null, v);
      stateIdx++;
    }
  }
  // Residual connection
...
export function runSEANetEncoder(
  ...
  for (let i = 0; i < 3; i++) {
    ...
    [x, states] = runSEANetResnetBlock(
      at(model, idx) as SEANetResnetBlock,
      states,
      x,
    );
    tree.dispose(states);
    ...
    const stride = at(ratios, i);
    [x] = runConv1d((at(model, idx) as StreamingConv1d).conv, null, x, stride);
    idx++;
  }
...
export const runSEANetDecoder = jit(function runSEANetDecoder(
  ...
  let idx = 1;
  for (let i = 0; i < 3; i++) {
    const blockState = at(state.blocks, i);
    // ELU
    x = nn.elu(x);
    idx++;
    ...
    [x, blockState.convtr] = runConvTranspose1d(
      (at(model, idx) as StreamingConvTranspose1d).convtr,
      blockState.convtr,
      x,
      stride,
    );
...
export function runMimiDecode(
  ...
  kvCacheLen += 16;
  offset += 16;
  const firstCache = at(kvCaches, 0);
  if (at(firstCache.key.shape, 0) !== 272) {
    // Pad it to a constant [272] in length, more than 250 context + 16 for next pass.
    const padAmount = 272 - at(firstCache.key.shape, 0);
...
export function runResBlock(
  ...
  const [shiftMlp, scaleMlp, gateMlp] = dims3Arrays(np.split(modulation, 3, -1));
...
export function createConv1dState(
  ...
  const [, cIn, kernelSize] = dims3(weight.shape);
  return np.zeros(
    [
      1, // batch size
      cIn, // in channels
      kernelSize - stride, // kernel size - stride
    ],
    { dtype: weight.dtype },
  );
}
...
export function runConv1d(
  ...
  state = x.ref.slice([], [], [at(x.shape, 2) - at(state.shape, 2)]);
...
export function createConvTranspose1dState(
  ...
  const [, cOut, kernelSize] = dims3(weight.shape);
  return np.zeros(
    [
      1, // batch size
      cOut * groups, // out channels
      kernelSize - stride, // kernel size - stride
    ],
    { dtype: weight.dtype },
  );
...
export function runConvTranspose1d(
  ...
  const [cIn, cOut, kernelSize] = dims3(weight.shape);
  ...
  y = y.add(np.pad(state, { 2: [0, at(y.shape, 2) - at(state.shape, 2)] }));
  const [head, tail] = dims2Arrays(np.split(y, [at(y.shape, 2) - at(state.shape, 2)], 2));
  y = head;
  state = tail;
  if (bias) y = y.add(np.expandDims(bias, -1));
  return [y, state];
